...
class userAuthController extends Controller {
  ...
  async checkOtp(req, res) {
    ...
    let WELLCOME_MESSAGE = `کد تایید شد، به کارجو خوش آمدید`;
    if (!user.isActive)
  ...
  sendOTP(phoneNumber, res) {
    ...
    kaveNegarApi.VerifyLookup(
      ...
      (response, status) => {
        ...
        console.log("kavenegar response:", JSON.stringify(response, null, 2));
        ...
        return res.status(status).send({
          statusCode: status,
          message: kavenegarMessage
            ? `کد اعتبارسنجی ارسال نشد: ${kavenegarMessage}`
